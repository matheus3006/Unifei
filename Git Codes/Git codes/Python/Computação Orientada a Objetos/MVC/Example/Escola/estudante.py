...
class CtrlEstudante():       
    ...
    def insereEstudantes(self):
        self.limiteInsc = LimiteInsereEstudantes(self)
    ...
    def cadastraHandler(self, event):
        ...
        if len(self.limiteInsc.inputNro.get()) == 0 or len(self.limiteInsc.inputNome.get()) == 0:
            ...
            nroMatric = self.limiteInsc.inputNro.get()
            nome = self.limiteInsc.inputNome.get()
            estudante = Estudante(nroMatric, nome)
            self.listaEstudantes.append(estudante)
            self.limiteInsc.mostraJanela('Sucesso', 'Aluno cadastrado!')
            self.clearHandler(event)

    def clearHandler(self, event):
        self.limiteInsc.inputNro.delete(0, len(self.limiteInsc.inputNro.get()))
